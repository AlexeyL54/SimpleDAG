...
│   ├── router.h        # Маршрутизация (если используется)
...
│   ├── router.cpp     # Реализация маршрутизации
...
- **Компилятор C++** с поддержкой C++17 (g++ 7.0+ или clang 5.0+)
...
После успешной сборки будет создан исполняемый файл `SimpleDAG.out`.
...
Тесты будут расположены в соответствующих поддиректориях `tests/`.
...
Поместите CSV-файл с данными и YAML-конфигурацию в директорию `data/`.
