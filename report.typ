...
+ Запустить программу SimpleDAG
  + Показать логотип приложения

+ #v(1em)
+ #line-label(<config-step>) ШАГ 1: Загрузка конфигурации
  + Получить список YAML-файлов в директории `data/`
  + *ЕСЛИ* файлов не найдено:
    + Вывести ошибку: "Не найдены конфигурационные файлы"
    + Завершить программу с кодом ошибки 1

  + #v(1em)
  + *ИНАЧЕ*:
    + Отобразить меню выбора файла с нумерацией
    + *ПОКА* выбор не валиден:
      + Запросить ввод номера файла от пользователя
      + *ЕСЛИ* ввод == 0:
        + Завершить программу (пользователь выбрал выход)
      + *ИНАЧЕ ЕСЛИ* номер вне диапазона:
        + Вывести сообщение об ошибке
        + Повторить запрос ввода
      + *ИНАЧЕ*:

+ #v(1em)
+ #line-label(<validation-step>) ШАГ 2: Проверка конфигурации
  + Проверить наличие указанных функций
  + *ЕСЛИ* найдены неизвестные функции:
    + Вывести список неизвестных функций
...
+ #v(1em)
+ #line-label(<data-step>) ШАГ 3: Загрузка данных
  + Сформировать полный путь к CSV: `data/` + имя_файла
  + Прочитать CSV-файл
  + *ЕСЛИ* файл не удалось открыть:
    + Вывести ошибку: "Не удалось открыть файл данных"
    + Завершить программу с кодом ошибки 1

  + #v(1em)
  + *ИНАЧЕ*:
    + Отобразить список доступных операций с номерами
    + *ДЛЯ* каждой операции в конфигурации:
      + Вывести номер: [N] ID_операции

+ #v(1em)
+ #line-label(<logging-step>) ШАГ 4: Инициализация логирования
  + Получить текущее время
  + Создать имя лог-файла с временной меткой
  + Открыть лог-файл для записи

+ #v(1em)
+ #line-label(<scheme-step>) ШАГ 5: Ввод и обработка схемы графа
  + Вывести инструкции по формату ввода схемы
  + *ПОВТОРЯТЬ* до получения валидной схемы:
    + Вывести приглашение "Ввод"
    + *ПОКА* не введена строка "end":
      + Считать строку из стандартного ввода
      + Добавить строку в вектор схемы

    + #v(1em)
    + Проверить схему
    + *ЕСЛИ* найдены ошибки:
      + Вывести сообщение об ошибке
      + *ДЛЯ* каждой строки с ошибками:
        + Вывести номер строки и текст ошибки
      + Очистить вектор схемы
      + Вывести "⚠️  Пожалуйста, исправьте ошибки и введите схему заново:"

    + #v(1em)
    + *ИНАЧЕ*:
      + Вывести "Схема графа корректна!"
      + Создать граф через
      + Прервать цикл проверки

+ #v(1em)
+ #line-label(<execution-step>) ШАГ 6: Выполнение операций
  + Выполнить обход графа в глубину через
    + *ДЛЯ* каждого узла графа в порядке обхода:
      + Получить ID операции из узла
      + Получить тип операции
      + Получить номер столбца
      + Определить тип данных столбца

      + #v(1em)
      + *ЕСЛИ* тип столбца == NUMERIC:
        + Считать числовые данные
        + Вызвать операцию
        + Записать результат в лог

      + #v(1em)
      + *ИНАЧЕ ЕСЛИ* тип столбца == STRING:
        + Считать строковые данные
        + Вызвать операцию
        + Записать результат в лог

      + #v(1em)
      + *ИНАЧЕ ЕСЛИ* тип столбца == UNKNOWN:
        + Записать предупреждение в лог
        + Пропустить эту операцию

+ #v(1em)
+ #line-label(<cleanup-step>) ШАГ 7: Завершение работы
  + Освободить ресурсы конфигурации
  + Очистить табличные данные
  + Закрыть лог-файл
  + Очистить граф:
  + Завершить программу с кодом 0 (успех)

+ #v(2em)
+ *Алгоритм проверки схемы*:
  + Вход: вектор строк схемы, вектор ID операций
  + *ДЛЯ* каждой строки в схеме:
    + Инициализировать вектор ошибок для строки
    + *ЕСЛИ* строка пустая: добавить ошибку "пустая строка"
    + *ЕСЛИ* разделитель "->" в начале или конце: добавить "неожиданный разделитель"
    + *ЕСЛИ* есть символы кроме цифр и "->": добавить "неожиданные символы"
    + Разделить строку на индексы по "->"
    + *ДЛЯ* каждого индекса:
      + Попытаться преобразовать в число
      + *ЕСЛИ* преобразование удалось *И* индекс вне диапазона [1, размер ID]:
        + Добавить ошибку "индекс вне диапазона"
  + Выход: вектор векторов ошибок для каждой строки
...
+ *Алгоритм создания графа*:
  ...
  + *ДЛЯ* каждой строки в схеме:
    ...
    + *ДЛЯ* каждого индекса i:
      ...
      + *ЕСЛИ* узел с таким ID еще не создан:
        + Создать новый узел
        + Добавить узел в граф
      + *ЕСЛИ* i > 0:
        + Создать связь между предыдущим и текущим узлом
  + Выход: построенный граф в глобальных переменных

+ #v(2em)
+ *Алгоритм определения типа столбца*:
  + Вход: номер столбца
  + *ДЛЯ* каждой строки в таблице:
    + *ЕСЛИ* столбец существует в строке:
      + *ЕСЛИ* значение является числом: поднять флаг "есть числа"
      + *ИНАЧЕ*: "есть строки"
  + *ЕСЛИ* "есть числа" - правда *И* "есть строки" - ложь: вернуть "ЧИСЛО"
  + *ЕСЛИ* поднят флаг "есть строки" вернуть "СТРОКА"
  + *ИНАЧЕ*: вернуть "НЕИЗВЕСТНО"
]
...
```cpp
float callOperation(const string &op_name, 
                    vector<float> arg);
```
*Описание* \
...
```cpp
string callOperation(const string &op_name, 
                     vector<string> arg);
```
*Описание* \
...
```cpp
Adjent *getPreviousAdjentInList(Node *node, 
                                Adjent *adjent);
```
*Описание* \
...
```cpp
vector<vector<SchenmeError>> checkScheme(
  vector<string> &scheme, vector<string> &ids);
```
*Описание* \
...
```cpp
bool safeStringToInt(const std::string &str, 
                     int &result);
```
*Описание* \
...
```cpp
int getValidatedNumberInput(const std::string &prompt, 
                            int min, int max);
```
*Описание* \
...
```cpp
bool validateAndDisplayScheme(vector<string> &scheme, 
                              vector<string> &ids);
```
*Описание* \
...
=== buildTests.sh
#listing("buildTests.sh", l: "bash")

=== runTests.sh
#listing("runTests.sh", l: "bash")

=== .github/workflows/ci.yml 
#listing(".github/workflows/ci.yml", l: "yml")
...
=== Проверка директории data на пустоту
  ...
  - *Ожидаемый результат:* Программа должна вывести сообщение "He найдены конфигурационные файлы в директории data".
  - *Фактический результат:* Сообщение выводится корректно.
  - *Изображение:*
    #pict("docs/img/251207_13h35m08s_screenshot.png")

=== Выбор файла конфигурации

  ==== Некорректный ввод (буквы, спецсимволы)
    ...
    - *Изображение:*
      #pict("docs/img/251207_19h24m34s_screenshot.png")
  ...
  ==== Введенное число вне диапазона
    ...
    - *Фактический результат:* Ошибка обрабатывается корректно.
    - *Изображение:*
      #pict("docs/img/251207_13h40m07s_screenshot.png")
  ...
  ==== Ввод "0" (выход)
    ...
    - *Изображение:*
      #pict("docs/img/251207_13h41m50s_screenshot.png")

=== Ввод схемы графа

  ==== Недопустимые символы (буквы, спецсимволы)
    ...
    - *Изображение:*
      #pict("docs/img/251207_13h56m26s_screenshot.png")

  ==== Разделитель в начале/конце строки
    ...
    - *Фактический результат:* Ошибка обрабатывается корректно.
    - *Изображение:*
      #pict("docs/img/251207_14h36m14s_screenshot.png")

  ==== ID операции вне диапазона
    ...
    - *Фактический результат:* Ошибка выводится корректно.
    - *Изображение:*
      #pict("docs/img/251207_13h57m44s_screenshot.png")
  ...
  ==== Пустая строка
    ...
    - *Фактический результат:* Ошибка обрабатывается корректно.
    - *Изображение:*
      #pict("docs/img/251207_13h59m57s_screenshot.png")

  ==== Корректный ввод схемы
    ...
    - *Изображение:*
      #pict("docs/img/251207_14h28m03s_screenshot.png")

=== Проверка конфигурации

  ==== Недоступная операция в YAML
    ...
    - *Фактический результат:* Ошибка обрабатывается корректно.
    - *Изображение:*
      #pict("docs/img/251207_14h26m14s_screenshot.png")
    - *YAML файл:*
```yaml
path: test.csv
...
```
  ...
  ==== Номер столбца вне диапазона
    ...
    - *Изображение:*
      #pict("docs/img/251207_14h30m32s_screenshot.png")
    - *YAML файл:*
```yaml
path: test.csv
...
=== Создание и выполнение графа
  ...
  ==== Граф без ветвлений
    ...
    - *Результаты выполнения:*
      ...
      ```
    - *Изображение:*
      #pict("docs/img/251207_14h40m37s_screenshot.png")
  ...
  ==== Граф с ветвлениями
    ...
    - *Изображение корректного ввода:*
      #figure(
      image("docs/img/251207_14h42m22s_screenshot.png", height: 95%),
      caption: [])
    ...
    - *Изображение результатов:*
      #pict("docs/img/251207_14h43m00s_screenshot.png")
...
=== Среда разработки
      #pict("docs/img/251207_15h03m38s_screenshot.png")
      #pict("docs/img/251207_15h03m57s_screenshot.png")
      #pict("docs/img/251207_15h04m25s_screenshot.png")
      #pict("docs/img/251207_15h04m48s_screenshot.png")
...
=== GitHub Actions (CI)
  ...
  - *Цель:* Обеспечить непрерывную интеграцию и проверку кода при обновлении веток: dev и main.
      #figure(
      image("docs/img/251207_14h54m54s_screenshot.png", height: 80%),
      caption: [])

      #figure(
      image("docs/img/251207_14h56m12s_screenshot.png", height: 80%),
      caption: [])

      #figure(
      image("docs/img/251207_14h57m04s_screenshot.png", height: 80%),
      caption: [])

      #figure(
      image("docs/img/251207_14h58m22s_screenshot.png", height: 80%),
      caption: [])
